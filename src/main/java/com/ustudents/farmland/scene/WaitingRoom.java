...
import com.ustudents.farmland.scene.menus.MultiplayerMenu;
import com.ustudents.farmland.scene.menus.SingleplayerMenu;
import imgui.ImGui;
...
public class WaitingRoom extends Scene {
    ...
    private void buttonForMultiPlayer(){
        if (ImGui.button("Multiplayer Mode")) {
            Game.get().getSceneManager().changeScene(new MultiplayerMenu());
        }
    }
    ...
    private void buttonForSinglePlayer(){
        if (ImGui.button("Singleplayer Mode")) {
            Game.get().getSceneManager().changeScene(new SingleplayerMenu());
        }
        ImGui.separator();
