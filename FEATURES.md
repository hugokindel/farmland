...
vidéo, voir modifier un peu le système de compilation pour le faire compiler dans son propre projet pour simplifier
cette usage.
...
à s'effectuer en seconde. Grâce à cette petite valeur, toute valeur multipliée par celle-ci se verra contrainte par le
...
console ainsi que du gras ou de l'italique (PS : Windows peute être capricieux avec ça).
...
utile pour retrouver une erreur qui s'est produite il y a quelques exécutions ou une autre information écrite.
...
### Les différents types d'éléments affichables
...
pour autant qu'il y a de fichiers de langues.
...
de la librairie ImGui qui permet de prototyper rapidement des interfaces complets sans avoir un système complexe de notre
côté.
...
- Thread d'interception client : attend de recevoir un message d'un client inconnu pour l'intercepter (récupérer ses
...
peut envoyer des messages à une adresse donnée et un port donné. Lors de l'envoi à une adresse ou un port différent,
...
- Thread d'interception serveur : attend de recevoir un message du serveur et le transmet pour ensuite être lu par le
  thread de lecture des messages.

### Format d'un message

Les messages sont envoyés sous forme de pseudo-JSON minimifié. Ce sont donc des objets Java sérialisés puis désérialisés.
...
Durant le tour du joueur, celui-ci aura le choix entre plusieurs actions :
...
Normal | Achète un terrain ou autant d'objets differents qu'il possède de terrain vide puis vend ce qu'il possède
Difficle | Achète un terrain ou autant d'objets identiques qu'il possède de terrain vide ou améliore ses recherches puis envoie des caravanes (s'il le peut) puis vend ce qu'il possède.
...
Il est important de noter que les touches relatives à la souris ne devraient pas être modifiées, et que les touches de la
souris ne peuvent pas être utilisé lors d'un changement de touche à l'heure actuelle.
...
--fast-bot | Rends le tour des robots immédiats
